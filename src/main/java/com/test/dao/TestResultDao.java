...
/*	@Query("Select t from TestResult t inner join  t.env_TestCase_TestResult e inner join TestCase tc inner join TestSuite ts inner join Run r where r.buildNumber = :buildNumber")
	...
	@Query("select tr from Run_TestCase_TestResult rtt join rtt.testResult tr join rtt.run run where run.environment.id = :envId and rtt.testCase.id = :testCaseId and tr.endTime > :dateBefore order by run.endTime asc")
